...
const char* NTC_MAGIC[NTC_MAGIC_COUNT] = {
    ...
    "M.K.", "M!K!", "PATT",    "NSMS", "LARD", "M&K!",
    "FEST", "N.T.", "M\0\0\0", "FA04", "FLT4", "EXO4",
    "TDZ4", ".M.K", "4CHN",    "04CH", "04CN"
};
...
void NTC_printcell(NTC_Display* context, const char* fmt, ...)
{
    ...
    NTC_print(
        stream, fname, "(PAT %03u ROW %02u CHN %01u) ", pat,
        row, (chn + 1)
    );
...
int NTC_checkmagic(NTC_File* file)
{
    ...
    if (freadres < 4)
    {
        NTC_print(
            stderr, file->name,
            feof(file->stream)
                ? "could not read the magic bytes (file ends "
                  "prematurely).\n"
    ...
    {
        if (!memcmp(magic, NTC_MAGIC[i], NTC_MAGIC_SIZE))
        {
            return 1;
        }
    }
...
int NTC_getpatnum(NTC_File* file)
{
    ...
    if (!freadres)
    {
        NTC_print(
            stderr, file->name,
            feof(file->stream)
                ? "could not read the number of orders (file "
                  "ends prematurely).\n"
    ...
    if (freadres < NTC_ORDNUM_MAX)
    {
        NTC_print(
            stderr, file->name,
            feof(file->stream) ? "could not read the orders "
    ...
    fseekres = fseek(
        file->stream, NTC_PATTERNS_PTR + patnum * NTC_PATSIZE,
        SEEK_SET
    );
    if (fseekres)
    {
...
int NTC_checkcmd(NTC_Pattern* context)
{
    ...
    {
        NTC_printcell(
            &context->display, "invalid command %01X.\n",
            (unsigned int)cmd
        );
    }
    else if (cmd == 0xD && prm)
    ...
    {
        NTC_printcell(
            &context->display, "CIA tempo command (F with "
                               "parameter larger than $1F).\n"
        );
    }
    else
    {
...
int NTC_procpat(NTC_File* file, size_t pat)
{
    ...
    fseek(
        file->stream, NTC_PATTERNS_PTR + pat * NTC_PATSIZE,
        SEEK_SET
    );
